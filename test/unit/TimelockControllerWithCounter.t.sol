...
contract TimelockControllerWithCounterTest is SimulationSetup {
    ...
    function test_MainnetSimpleVote() public {
        ...
        bytes32 txId = timelock(srcChain).hashOperation(
            address(governor()), 0, abi.encodeWithSelector(governor().updateQuorumNumerator.selector, 11), bytes32(0), 0
        );
    ...
    function test_RevertWhen_NotProposerScheduleSingle() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IAccessControl.AccessControlUnauthorizedAccount.selector, alice, timelock(srcChain).PROPOSER_ROLE()
            )
        );
        vm.prank(alice);
    ...
    function _decodeLzReceivePayload(bytes memory payload, uint256 chainId, bool isBatch)
        internal
        view
        returns (bytes32)
    {
    ...
    function _decodePayloadTimelockScheduleBatch(bytes calldata payload, uint256 chainId)
        public
        view
        returns (bytes32 id)
    {
